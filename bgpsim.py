...
class Router:
    ...
    def withdraw(self, rt):
        ...
        if prefix in self.rib:
            self.rib[prefix] = [route for route in self.rib[prefix] if route.neighbor != rt.neighbor]
    ...
    def next_hop(self, ipaddr):
        ...
        # YOUR CODE HERE
        ip_binary = self.convertToBinaryString(ipaddr)

        for prefix_key, routes in self.rib.items():
            prefix, prefix_len = prefix_key.split('/')
            prefix_len = int(prefix_len)
            prefix_binary = self.convertToBinaryString(prefix)

            if ip_binary.startswith(prefix_binary[:prefix_len]):
                if prefix_len > best_prefix_len:
                    retval = min(routes, key=lambda r: len(r.path))
                    best_prefix_len = prefix_len
                elif prefix_len == best_prefix_len:
                    best_route = min(routes, key=lambda r: len(r.path))
                    if retval is NONE or len(best_route.path) < len(retval.path):
                        retval = best_route

        if retval:
            return retval.neighbor
